const App = {
    ...
    // ==========================================
    // 🗄️ MASTER DATA MERGE ENGINE ("Source of Truth" Architecture)
    // ==========================================
    processSyncData(jsonText) {
        try {
            const importedData = JSON.parse(jsonText);
            
            // STEP 1: Merge the Receipts (Journal) First
            // This guarantees zero duplicates using unique millisecond IDs
            const currentLogs = typeof Journal !== 'undefined' ? Journal.getAllLogs() : [];
            const importedLogs = (importedData.journal && Array.isArray(importedData.journal)) ? importedData.journal : [];
            const combined = [...currentLogs, ...importedLogs];
            ...
            // STEP 2: Rebuild the Scoreboard (History) from Scratch
            // We throw away the imported/current history numbers and blindly trust the Journal
            let rebuiltHistory = {};
            finalJournal.forEach(log => {
                // Safeguard: Only sum up actual focus time (ignore saved breaks)
                if (log.bucket !== 'Recovery' && log.quality !== 'break') {
                    if (!rebuiltHistory[log.date]) rebuiltHistory[log.date] = 0;
                    rebuiltHistory[log.date] += log.duration;
                }
            });

            // Overwrite the database with our perfectly calculated history
            Storage.set(Storage.KEYS.HISTORY, rebuiltHistory);
            
            // STEP 3: Update Today's Live Stats on the Home Screen
            const todayStr = new Date().toDateString();
            const todayTotal = rebuiltHistory[todayStr] || 0;
            ...
            // Force Insights tab to immediately redraw the charts with the new math
